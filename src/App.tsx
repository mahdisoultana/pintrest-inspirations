...
function OverlayCard({ setSel }: { setSel: (pram: boolean) => void }) {
  ...
  const animate = (dir = 1, anime?: any) => ({
    opacity: status == 'notHover' ? 1 : 0,
    x: status == 'notHover' ? '0%' : dir > 0 ? '-200%' : '200%',
    transition: { delay: status == 'notHover' ? 0.4 : 0 },
    ...anime,
  });
  const hoverEffect = {
    rotate: 375,
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={` absolute top-0 object-cover left-0 w-full h-full bg-gradient-to-l duration-1000 ${
          status == 'hover'
            ? 'to-black/0 from-transparent'
            : 'from-black/80 to-transparent'
        } z-10`}
      />
      <PlayButton onClick={() => setSel(false)} />
      <motion.div
        initial="initial"
        animate="animate"
        variants={variantsC}
